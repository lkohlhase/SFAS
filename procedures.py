...
def comparetotruth(truth,severalsits,scalerino2):
    ...
    for i in range(halfway):
        boundaries,label1,label2=severalsits[i]
        for j in range(len(boundaries[:-1])):
            ...
            ax.text(-scalerino2, i + 0.33, label1, size='48')
            ax.text(truthlength,i+0.33,label2,size='48')

    truthranges,label1,label2=truth
    ...
    ax.text(-scalerino2,halfway+0.33,label1,size='48')
    ax.text(truthlength, halfway+ 0.33, label2, size='48')
    for i in range(len(severalsits)-halfway):
        boundaries, label1,label2 = severalsits[i+halfway]
        for j in range(len(boundaries[:-1])):
            scalerino = truthlength/float(boundaries[-1])
            ...
            ax.text(-scalerino2, i+1+halfway + 0.33, label1, size='48')
            ax.text(truthlength,i+1+halfway + 0.33,label2,size='48')
    ax.set_xlim(-scalerino2,truthlength+scalerino2)
    ax.set_ylim(0,len(severalsits)+1)
    ax.get_yaxis().set_visible(False)
    for tick in ax.xaxis.get_major_ticks():
        tick.label.set_fontsize(20)
    plt.xlabel('Time t',fontsize=24)
# asdf=([1,100,200,300,400],'label1','Quality')
# asdf2=([1,150,250,450,500],'label2','Quality')
# truth=([(1,(1,1)),(125,(100,150)),(225,(200,250)),(450,(450,450))],'truth','')
# comparetotruth(truth,[asdf,asdf2],100)
# plt.show()